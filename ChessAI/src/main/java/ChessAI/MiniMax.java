...
import ChessAI.Coordinate;
import ChessAI.GameTest;
import ChessAI.Move;
import java.util.ArrayList;
/*
public class MiniMax {

    public static boolean timeDebug =  false;//GameTest.ENABLE_AI_TIMER && GameTest.ENABLE_MOVE_ORDERING; // For computation time metrics.
    private int orderTimeSum = 0;
    private int orderTimeCount = 0;

    public UtilMove nextMove(Board boardIn, String color, int maxDepth) throws Exception {
        Board board = new Board(boardIn.board);
        orderTimeSum = 0;
        orderTimeCount = 0;
        ...
        for (Move move : moves) {
            Board temp = new Board(board,move);
            int score = min(temp.getNextPlayer(), temp, maxDepth, alpha, beta); //Next player or current played
    ...
    private int min(String player, Board board, int depth, int alpha, int beta) throws Exception{
        if (board.isGoalState(player) || depth <= 0) {
            return board.evaluationFunction(Board.getNextPlayer(player));
        }

        ArrayList<Board> validBoards = new ArrayList<>();
        for (Move move : board.getMoves(board.getCurrentPlayer())) {
            validBoards.add(new Board(board, move));
        }

        if (GameTest.ENABLE_MOVE_ORDERING) {
    ...
    private int max(String player, Board board, int depth, int alpha, int beta) throws Exception{
        if (board.isGoalState(player) || depth <= 0) {
            return board.evaluationFunction(Board.getNextPlayer(player));
        }
    ...
    private ArrayList<Board> orderMoves(ArrayList<Board> boards, String player){
    ...
    private class SortingAlgorithms {
        ...
        public void quickSort(Board arr[], int begin, int end, String player) {
            if (begin < end) {
        ...
        private int partition(Board arr[], int begin, int end, String player) {
            Board pivot = arr[end];
            int i = (begin - 1);
...
public class MiniMax {
    ...
    public static boolean timeDebug = GameTest.ENABLE_AI_TIMER && GameTest.ENABLE_MOVE_ORDERING; // For computation time metrics.
    private int orderTimeSum = 0;
    private int orderTimeCount = 0;
    ...
    public UtilMove nextMove(Board board, int player, int maxDepth) {
        ...
        UtilMove bestMove = new UtilMove(Integer.MIN_VALUE, null);
        ArrayList<Coordinate> moves = board.getPossibleBoards();
        int alpha = Integer.MIN_VALUE;
        int beta = Integer.MAX_VALUE;
        ...
        if (timeDebug) {
            long stopTime = System.currentTimeMillis();
            long elapsedTime = stopTime - startTime;8
            System.out.println("Total time for move decision: " + (elapsedTime / 1000) + " seconds");
            System.out.println("Total time on move ordering: " + (orderTimeSum) + " milliseconds (" + (orderTimeSum / 1000) + " seconds)");
            System.out.println("Average time on move ordering: " + (orderTimeSum / (orderTimeCount)) + " milliseconds per iteration");
        }
